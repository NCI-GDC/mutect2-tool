...
def do_pool_commands(cmd, case_id, engine, logger, files, lock = Lock()):
    ...
    with lock:
        ...
        cmd_list = cmd.split()
        toolname = ('mutect2_variant_call: %s' % cmd_list[15])
...
def mutect2_cmd_template(gatk_path, ref, fai_path, blocksize, java_heap, cosmic, dbsnp, pon, output_base, contEst, normal_bam_path, tumor_bam_path):
    template = string.Template("/usr/bin/time -v java -Djava.io.tmpdir=/tmp/job_tmp -d64 -jar -Xmx${JAVA_HEAP} ${GATK_PATH} -T MuTect2 -nct 1 -R ${REF} -L ${REGION} -I:tumor ${TUMOR_BAM} -I:normal ${NORMAL_BAM} --cosmic ${COSMIC} --dbsnp ${DBSNP} --normal_panel ${PON} --contamination_fraction_to_filter ${CONTAMINATION} -o ${OUTPUT_BASE}.${BLOCK_NUM}.mt2.vcf")
...
def run_mutect(case_id, normal_id, normal_bam_path, tumor_id, tumor_bam_path, thread_count, java_heap, reference_fasta_path, contEst, cosmic_path, pon_path, fai_path, blocksize, known_snp_vcf_path, engine, logger):
